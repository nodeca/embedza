...
Embedza.prototype.info = function (url, callback) {
  ...
  this.__options__.cache.get(url, (err, data) => {
    ...
    // Run fetchers
    async.eachSeries(this.__rulesCache__.domains[env.result.domain].fetchers, (fetcher, next) => {
      fetcher.fn(env, next);
    }, err => {
      if (err) {
        callback(err);
      ...
      // Run mixins
      async.eachSeries(this.__rulesCache__.domains[env.result.domain].mixins, (mixin, next) => {
        mixin(env, next);
      }, err => {
        if (err) {
          callback(err);
        ...
        // Run mixins after
        async.eachSeries(this.__rulesCache__.domains[env.result.domain].mixinsAfter, (mixinAfter, next) => {
          mixinAfter(env, next);
        }, err => {
          if (err) {
            callback(err);
          ...
          this.__options__.cache.set(url, { info: env.result, ts: Date.now() }, err => {
            if (err) {
              callback(err);
              return;
            }
