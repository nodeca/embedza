...
fetchers.push({
  id: 'meta',
  priority: -100,
  fn: Promise.coroutine(function* meta_fetcher(env) {
    debug('meta');
    ...
    try {
      response = yield env.self.request(env.src);
    } catch (err) {
      if (err.statusCode) {
        throw new EmbedzaError(
          `Meta fetcher: Bad response code: ${err.statusCode}`,
          'EHTTP',
          err.statusCode);
      }
      throw err;
...
fetchers.push({
  id: 'oembed',
  fn: Promise.coroutine(function* oembed_fetcher(env) {
    debug('oembed');
    ...
    try {
      response = yield env.self.request(oembed[0]);
    } catch (err) {
      if (err.statusCode) {
        throw new EmbedzaError(
          `Oembed fetcher: Bad response code: ${err.statusCode}`,
          'EHTTP',
          err.statusCode);
      }
      throw err;
