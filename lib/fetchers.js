  ...
  fn: function meta_fetcher(env, callback) {
    ...
    env.self.request(env.src, function (err, response, body) {
      ...
      if (response.statusCode !== 200) {
        err = new EmbedzaError('Meta fetcher: Bad response code: ' + response.statusCode);
        err.code = 'EHTTP';
...
fetchers.push({
  ...
  fn: function oembed_fetcher(env, callback) {
    ...
    env.self.request(oembed[0], function (err, response, body) {
      ...
      if (response.statusCode !== 200) {
        err = new EmbedzaError(new EmbedzaError('Oembed fetcher: Bad response code: ' + response.statusCode));
        err.code = 'EHTTP';
        err.status = response.statusCode;
        callback(err);
        return;
      ...
      if (response.headers['content-type'].indexOf('application/json') === 0) {
        try {
          ...
          err = new EmbedzaError(new EmbedzaError('Oembed fetcher: Can\'t parse oembed JSON response'));
          err.code = 'ECONTENT';
          callback(err);
          return;
        }
      ...
      err = new EmbedzaError('Oembed fetcher: Unknown oembed response content-type:' +
