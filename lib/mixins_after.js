...
var size         = require('probe-image-size');
...
mixinsAfter.push({
  ...
  fn: function mime_detect_after_mixin(env, callback) {
    ...
    async.each(env.result.snippets, function (snippet, next) {
      ...
      env.self.request(snippet.href, { method: 'HEAD' }, function (err, response) {
...
function loadImageSize(url, cache, callback) {
  ...
  cache.get(cacheKey, function (err, data) {
    ...
    size(url, function (err, dimensions) {
      if (err) {
        callback(err);
        return;
      }
      ...
      // Save data to cache
      cache.set(cacheKey, { dimensions: dimensions, ts: Date.now() }, function (err) {
        if (err) {
          callback(err);
          return;
        }
...
mixinsAfter.push({
  ...
  fn: function image_size_after_mixin(env, callback) {
    ...
    if (!env.self.__loadImageSize__) {
      env.self.__loadImageSize__ = utils.uniqueAsync(function (url, callback) {
        loadImageSize(url, env.self.__options__.cache, callback);
      });
    }

    var supportedExt = [ '.bmp', '.gif', '.jpg', '.jpeg', '.png', '.psd', '.tif', '.tiff', '.webp' ];
    ...
    async.each(env.result.snippets, function (snippet, next) {
      ...
      env.self.__loadImageSize__(snippet.href, function (err, dimensions) {
...
// Convert 'width', 'height' and 'duration' to integer
//
mixinsAfter.push({
  id: 'convert-str-int',
  fn: function convert_str_int_after_mixin(env, callback) {
    ...
    var fields = [ 'width', 'height', 'duration' ];

    env.result.snippets.forEach(function (snippet) {
      fields.forEach(function (field) {
        if (snippet.media[field]) {
          snippet.media[field] = parseInt(snippet.media[field], 10);
        }
      });
    });
