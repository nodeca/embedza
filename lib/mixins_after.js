...
function loadImageSize(url, cache) {
  ...
  return cache.get(cacheKey).then(data => {
    ...
    return size(url).then(dimensions => {
...
mixinsAfter.push({
  ...
  fn: Promise.coroutine(function* image_size_after_mixin(env) {
    ...
    for (let snippet of queue) {
      let size = dimensions[snippet.href];

      if (!size) continue;

      if (size.wUnits !== 'px' || size.hUnits !== 'px') continue;

      snippet.media = snippet.media || {};

      snippet.media.width  = size.width;
      snippet.media.height = size.height;
    }
