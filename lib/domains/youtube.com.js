...
var tpl = url.parse('http://www.youtube.com/oembed?format=json&url=foobar', true, true);
delete tpl.search;
...
module.exports = {
  ...
  fetchers: [
    function youtube_fetcher(env, callback) {
      var urlObj = url.parse(env.src, true);
      ...
      env.self.request(url.format(tpl), function (err, response, body) {
        ...
        try {
          ...
          body = body.replace(/(\\+)(U[0-9a-fA-F]{8})/g, function (match, slashes, escape) {
            ...
            var code = parseInt(escape.slice(1), 16);
          ...
          err = new EmbedzaError(new EmbedzaError('YouTube fetcher: Can\'t parse oembed JSON response'));
          err.code = 'ECONTENT';
          callback(err);
          return;
        }
