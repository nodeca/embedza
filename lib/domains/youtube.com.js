    ...
    function youtube_fetcher(env, callback) {
      ...
      env.self.request(url.format(tpl), function (err, response, body) {
        ...
        if (response.statusCode !== 200) {
          callback(new EmbedzaError(`YouTube fetcher: Bad response code: ${response.statusCode}`, 'EHTTP', response.statusCode));
          return;
        }
        ...
        try {
          ...
          callback(new EmbedzaError("YouTube fetcher: Can't parse oembed JSON response", 'ECONTENT'));
          return;
        }
